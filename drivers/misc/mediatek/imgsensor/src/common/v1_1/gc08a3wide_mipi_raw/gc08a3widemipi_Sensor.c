...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC08A3WIDE_SENSOR_ID,
...
static kal_uint16 gc08a3wide_1632x1224_addr_data[] = {
/*system*/
	0x031c, 0x60,
	0x0337, 0x04,
	0x0335, 0x55,
...
static kal_uint16 gc08a3wide_3264x2448_addr_data[] = {
/*system*/
	0x031c, 0x60,
	0x0337, 0x04,
	0x0335, 0x51,
...
static kal_uint16 gc08a3wide_1280x720_addr_data[] = {
/*system*/
	0x031c, 0x60,
	0x0337, 0x04,
	0x0335, 0x55,
...
static void preview_setting(void)
{
	...
	table_write_cmos_sensor(gc08a3wide_1632x1224_addr_data,
		sizeof(gc08a3wide_1632x1224_addr_data)/sizeof(kal_uint16));
...
static void capture_setting(kal_uint16 currefps)
{
	...
	table_write_cmos_sensor(gc08a3wide_3264x2448_addr_data,
		sizeof(gc08a3wide_3264x2448_addr_data)/sizeof(kal_uint16));
...
static void slim_video_setting(void)
{
	...
	table_write_cmos_sensor(gc08a3wide_1280x720_addr_data,
		sizeof(gc08a3wide_1280x720_addr_data)/sizeof(kal_uint16));
	...
	struct SET_SENSOR_PATTERN_SOLID_COLOR *pdata)
{
	...
	if (modes) {
		if (modes == 5 && (pdata != NULL)) {
			...
			write_cmos_sensor_8bit(0x50C1, 0x01);
	} else {
		...
		do {
			*sensor_id = return_sensor_id() + 2;
			if (*sensor_id == imgsensor_info.sensor_id) {
				pr_debug("[gc08a3wide_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id);
				return ERROR_NONE;
			}
			pr_debug("[gc08a3wide_camera_sensor]get_imgsensor_id:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id() + 2 ;
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_debug("[gc08a3wide_camera_sensor]open:i2c write id: 0x%x, sensor id: 0x%x\n",
					imgsensor.i2c_write_id, sensor_id);
				break;
			}
			pr_debug("[gc08a3wide_camera_sensor]open:Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
